...
out$.Guide = Guide = (function(superclass){
  ...
  prototype.positionFlare = function(flare, brickHeight, dropDistance){
    var g;
    g = this.opts.gridSize;
    flare.scale.y = g * (1 + dropDistance) / this.height;
    return flare.position.y = this.height - g * brickHeight - g * dropDistance;
  };
  prototype.showFlare = function(p, dropDistance){
    var beamShape, x, y;
    if (true) {
      this.state.lastShape = beamShape = this.state.thisShape;
      x = this.positionBeam(this.flare, beamShape);
      y = this.positionFlare(this.flare, beamShape.height, dropDistance);
      this.flare.material.materials.map(function(it){
        var ref$;
        return (ref$ = it.emissive) != null ? ref$.setHex(beamShape.color) : void 8;
      });
      this.showImpactLight(x, beamShape);
    }
    return this.impactLight.intensity = 10 * (1 - p);
  };
  prototype.showImpactLight = function(x, beamShape){
    this.impactLight.hex = beamShape.color;
    this.impactLight.position.x = x;
    return this.impactLight.position.y = this.height - this.opts.gridSize * beamShape.height;
  };
  return Guide;
}(Base));
function extend$(sub, sup){
